...
class Day05 {
    ...
    static function part2a(input:Array<String>):Float {
            ...
            // while (seeds.length > 0) {
                ...
                if (map[1] <= seed && seed < map[1] + map[2]) {
                    ...
                    if (seed + length >= map[1] + map[2]) { //length not in range                        
                        ...
                        // var newLength = seeds[j+1] - (map[2] - (map[1]-map[0]));
                        var newLength = seed + length - (map[1] + map[2]);
                        trace('AAA: $newSeed, $newLength');

                        newSeeds.push(newSeed);
                        newSeeds.push(newLength);
                        
                        newSeeds.push(map[1] + map[2] - seed);
                        newSeeds.push(map[0] - map[1] + seed);

                    } else { // length in range
                        newSeeds.push(map[0] - map[1] + seed);
                        newSeeds.push(length);
                    }
                } else {
